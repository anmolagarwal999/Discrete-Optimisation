/*
	 Javascipt to retrieve input file, output file in TSP solver and then creating corresponding graph
*/
...
// Create the graph with only the points with proper scaling
...
// Add edges to the graph depending on the solution file
...
// Function to deal with event of input file being dropped into the dropbox
function loadBenchmark(text){
	...
	if(benchmark != null){
		...
		$.ajax({
  			...
  			data : JSON.stringify({'data': text, 'data1': strusr}), // Send data as JSON variable
  			dataType: "json",
  			success:function(data) {
      			if(data.redirect)
      			{
					window.location.href = data.redirect; // On success redirect to url returned by flask function
					return false;
      			}
    		}
...
// Function to deal with event of solution file being dropped into the dropbox
...
// Check validity of Solution file.
